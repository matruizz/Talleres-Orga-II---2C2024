...
Nos referimos a el modo de operacion de un procesador de intel, el modo protegido es el modo de operación nativo y el modo de operación de direccionamiento real, este modo provee el ambiente de programación igual al de el procesador 8086 de intel.
El modo real es el modo en que todo procesador con la arquitectura x86 arranca, tiene un direccionamiento de 16 bits, por lo que puede direccionar solo a 1 MiB de memoria, no tiene protección por lo que no tiene niveles de privilegio, todos los usuarios tienen acceso a todas las instrucciones y toda la memoria.
En cambio el modo protegido tiene direccionameinto de 4 GiB, tiene 4 niveles de privilegio, por lo que cada usuario tiene acceso a un conjunto de instrucciones que depende del nivel de privilegio que tiene.
...
Si podemos tener un sistema operativo en modo real como por ejemplo DOS, pero seria muy limitado.
Podria tener solo un 1 MiB de memoria y no tendria niveles de protección, .
...
3. Busquen el manual *volumen 3 de Intel en la sección 3.4.5 Segment Descriptors*. ¿Qué es la GDT? ¿Cómo es el formato de un descriptor de segmento, bit a bit? Expliquen brevemente para qué sirven los campos *Limit*, *Base*, *G*, *P*, *DPL*, *S*. También pueden referirse a los slides de la clase teórica, aunque recomendamos que se acostumbren a consultar el manual.
    La gdt es la estructura de memoria en donde están almacenados los descriptores de segmento globales.
    0  - 15 : Límite 0:15: El tamaño del segmento en unidades de bytes o 4 kib menos 1 dependiendo de la granularidad
    16 - 31 : direccion base 0:15: La direccion de memoria donde empieza el segmento.
    32 - 39 : direccion base 16:23
    40 - 42 : tipo
    43      : S: Indica se el segmento es de systema o de (codigo o datos).
    44 - 45 : DPL: Es nivel de privilegio del segmento.
    46      : P: Es 1 si el segmento está cargado en memoria y 0 sino  
    47 - 50 : Limite
    51      : AVL
    52      : L
    53      : D/B
    54      : G: Determina la escala de la unidad del campo limite, si esta en 0 es bytes si esta en 1 es 4 KiB. 
    55 - 61 : Base 24:31
...
Tiene la gdt que es un array que contine todos los descriptores de segmento. El GDT_DESC contiene el segment selector y el offset.
...
Escribe caracteres en la seccion de memeoria destinada a la pantalla.  
...
DUDAS 
D/B: Estos descriptores de segmento son para segmentos de 32 o de 16 y ese existe la segmentacion en modo real?.....
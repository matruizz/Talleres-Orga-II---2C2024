...
typedef struct nodo_s {     // Offset  Padding
    struct nodo_s* next;    //      0        0 Siguiente elemento de la lista o NULL si es el final
    uint8_t categoria;      //      8        7 Categoría del nodo
    uint32_t* arreglo;      //     16        0 Arreglo de enteros
    uint32_t longitud;      //     24        4 Longitud del arreglo
} nodo_t;                   //        TOTAL 32

typedef struct __attribute__((__packed__)) packed_nodo_s {  // Offset   Padding
    struct packed_nodo_s* next;                             //      0         0 Siguiente elemento de la lista o NULL si es el final
    uint8_t categoria;                                      //      8         0 Categoría del nodo
    uint32_t* arreglo;                                      //      9         0 Arreglo de enteros
    uint32_t longitud;                                      //     17         0 Longitud del arreglo
} packed_nodo_t;                                                    //TOTAL  21

typedef struct lista_s {    //Offset    Padding
    nodo_t* head;           //     0          0 Puntero al primer nodo que encabeza la lista
} lista_t;

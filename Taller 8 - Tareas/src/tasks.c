...
static int8_t create_task(tipo_e tipo) {
  size_t gdt_id;
  for (gdt_id = GDT_TSS_START; gdt_id < GDT_COUNT; gdt_id++) {  //Busca la primer entrada de la gdt que esta vacia
    if (gdt[gdt_id].p == 0) {
      break;
    }
  }
  kassert(gdt_id < GDT_COUNT, "No hay entradas disponibles en la GDT"); //Se asegura que la entrada sea valida 

  int8_t task_id = sched_add_task(gdt_id << 3); //Agrega la tarea al scheduler y devuelve el task_id
  tss_tasks[task_id] = tss_create_user_task(task_code_start[tipo]); //Agrega el nuevo tss al arreglo de tss
  gdt[gdt_id] = tss_gdt_entry_for_task(&tss_tasks[task_id]);  //agrega el selector de tss a la gdt
  return task_id; //Devuelve el task id asignado por el scheduler
}
...
void tasks_init(void) {
  ...
  task_id = create_task(TASK_A);  //Crea una tarea de tipo A -> agregar la tarea al scheduler, crea su tss y cargar el selector de tss a la gdt
  sched_enable_task(task_id);     //Pone el estado de la tarea en runnable

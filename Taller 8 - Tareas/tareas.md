...
la inicial y la iddle, tenemos que configurar dos tss, una para cada tarea, 
ademas sus respectivos descriptores de la gdt, aparte de eso tenemos que
inicializar el directorio la tabla de paginas que permitan acceder a 2 paginas
de codigo, 1 de stack y otra para la memoria compartida on demand.
Ademas para definir la tss, tenemos que configurar el redistro tr, el cual es
registro de 16 bits oculto que guarda el selector de segmento que permite acceder
...
Por carias razones por ejemplo como ya estan activadas las interrupciones, puede ocurrir una y el kernel tendra que ocuparse de su ejecucion, en dicho caso hay que tener inicializada una pila de nivel 0 para que el kernel funcione.
...
_isr32:
  pushad                Se pushean al stack todos los registros de proposito general
  call pic_finish1      Se avisa al pic que la interrupcion ya fue atendida
  
  call sched_next_task  funcion que devuelve el selector de segmento de la siguiente tarea a ser ejecutada
  
  str cx                str carga en cx el valor presente el el registro tr
  cmp ax, cx            compara los valores del registro tr actual con el de la siguiente tarea a ejecutarse
  je .fin               si son iguales salta al fin, es decir si el scheduler decide que la tarea actual se 
                        tiene que seguir ejecutando se sale de la rutina de atencion al reloj y se sigue con lo que se estaba haciendo antes de la interrupcion
  
  mov word [sched_task_selector], ax si los selectores fueron distintos se mueve a la direccion identity mapeada 4kb desde
                                  el inicio el valor correspondiente al selector de tss de la nueva tarea a ejecutarse
  jmp far [sched_task_offset]   Se ejecuta un jump far que modifica el valor del registro tr y provoca un cambio de tarea

  .fin:
  popad                 Se popean los registros de proposito general
  iret                  La ejecucion vuelve a donde estaba antes de la interrupcion del reloj, esto aplica tanto para
                        cuando ocurre un cambio de tarea como cuando no
```
...
Es de 48 bytes 32 del offset y 16 del selector de tss, el offset elegido no tiene ningun efecto pues
la unidad de segmentacion al darse cuenta que el selector de segmento apunta a un descriptor de tss 
y no a un descriptor de segmento "regular" sabe que lo que esta ocurriendo no es un cambio en la flujo 
de ejecucion del codigo sino que lo que esta ocurriendo es un cambio de contexto. 

c)  ¿A dónde regresa la ejecución (***eip***) de una tarea cuando
    vuelve a ser puesta en ejecución?descriptor

Vuelve a la siguiente instrucion que iba a ejecutarse antes del cambio de contexto, es decir a la siguiente
instruccion despues del jmp dentro de la rutina de atencion al reloj, en el caso de la rutina de atencion al
reloj que tenemos en nuestro sistema vuelve a la etiqueta .fin
...
Recorre los id de tareas desde el siguiente a la actual hasta volver al id de la actual o encontrar el id
de una tarea que esta en estado ejecutable, si encuentra una tarea en estado ejecutable devuelve su selector de tss
si no encuentra ninguna tarea en estado ejecutable devuelve el selector de tss de la tarea idle.
...
porque lo que recibe la funcion como parametro es el selector de segemtno que tiene el indice correspondiente al segmenteo de la gdt a partir del bit 3 y el los bits 0 1 2 tiene el rpl y el table indicator estos 3 ultimos bits tiene que ser ceros
para indicar la tabla gdt y nivel de privilegio 0
